...
public class GUI {
	...
	/**
	 * Creates the frame, requiring a minimum size defined as the maximum size of
	 * the frame of the actual workspace.
	 */
	...
	/**
	 * Creates the left and right panel, adding them to the frame. It also creates
	 * an array of buttons that will be enabled or disabled and JTextFields that
	 * could be altered in the future by the user.
	 */
	 ...
	 * feautureEnvyButtonPanel panel and indicatorsTable panel with all the labels,
	 * text field boxes, buttons and option panes associated to them. In the
	 * fileChoser panel, when the load button is selected it opens an Excel file,
	 * while when the show button is selected it opens the Excel program with the
	 * selected file. In the parametrosLongMethodPanel panel, the JLabel linesOfCode
	 * as well as the JLabel cyclomaticComplexity only accept numbers in its
	 * argument. The JPanel longMetodPanel creates the button long method which when
	 * pressed, will execute the long method and the defect detection to the values
	 * previously defined. The JPanel botoesFeatureEnvyPanel creates the JLabel atfd
	 * label and the JLabel laa label and posteriorly changes the JTextField laa_txt
	 * and JTextField atfd_txt, accepting integer or double numbers. In The JPanel
	 * feautureEnvyButtonPanel the button Feature Envy is created which will execute
	 * the feature envy and the defect detection for the values defined by the user.
		...
		createButton("Feature Envy", 400, 50, 15, 2).addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (!atfd_txt.getText().toString().equals("") && !laa_txt.getText().toString().equals("")) {
					...
					app.feature_envy(Double.parseDouble(atfd_txt.getText()), Double.parseDouble(laa_txt.getText()),
							choice);
					fillFeature_envy();
	...
	/**
	 * Creates and adds a table to a determined panel. The definedRules is the only
	 * label in which the font is equal to eleven. If so the label points to the
	 * definedRules.
	 * 
	...
	/**
	 * In the array textFields it is created a JTextField and adds it to the
	 * corresponding table.
	 * 
	...
	/**
	 * Right part of the frame. It is create a String array with all the names for
	 * the column and then adds it to the frame, posteriorly setting the columns
	 * sizes, and its corresponding maximum and minimum widths.
	 * 
	...
	/**
	 * Firstly, sets the row count to 0 and after it fills the table accordingly,
	...
	/**
	 * Fills the column of the is feature envy by iterating the getsMethodsData and
	 * posteriorly setting the corresponding values.
	 */
	...
	/**
	 * Receives and places on the Graphical User Interface (GUI) the results from
	 * the rules defined.
	 * 
